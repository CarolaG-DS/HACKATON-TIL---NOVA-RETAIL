...
SELECT
    ...
    -- Parsing flessibile delle date con fallback per preservare il 100% delle righe
    COALESCE(
        CASE 
        ...
        END,
        '1970-01-01'::DATE -- Evita NULL mantenendo la riga per i test dbt
    ) AS transaction_date,
    ...
    -- Pulizia Prezzo Robusta (rimuove simboli valuta, virgole delle migliaia e spazi)
    CAST(
    ...
    -- Normalizzazione sconto (Corretto: '% %' o LIKE '%')
    CAST(
        CASE 
            WHEN "discount_pct" ILIKE '%N/A%' OR "discount_pct" IS NULL OR TRIM("discount_pct") = '' THEN 0.00
            WHEN "discount_pct" LIKE '%\%' OR "discount_pct" LIKE '%%' THEN CAST(REPLACE(REPLACE("discount_pct", '%', ''), ' ', '') AS NUMBER(5,2)) / 100.0
